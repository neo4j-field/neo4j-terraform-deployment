...
| <a name="input_bloom_license"></a> [bloom\_license](#input\_bloom\_license) | License key for the Neo4j Bloom plugin | `string` | `"bloom.txt"` | no |
| <a name="input_bloom_version"></a> [bloom\_version](#input\_bloom\_version) | Neo4j Bloom version to be installed | `string` | `"1.9.0"` | no |
| <a name="input_bucket_name"></a> [bucket\_name](#input\_bucket\_name) | Name of the GCS bucket to be used by this Terraform deployment | `string` | `"terraform-neo4j-causal-cluster-deployer-01"` | no |
...
| <a name="input_dbms_mode"></a> [dbms\_mode](#input\_dbms\_mode) | DBMS mode of the node being provisioned | `string` | `"CORE"` | no |
...
| <a name="input_labels_group"></a> [labels\_group](#input\_labels\_group) | Group labels used by this Terraform deployment | `string` | `"neo4j-causal-cluster"` | no |
| <a name="input_machine_type"></a> [machine\_type](#input\_machine\_type) | Machine type of the VM being provisioned by this Terraform deployment | `string` | `"e2-medium"` | no |
| <a name="input_max_heap_size"></a> [max\_heap\_size](#input\_max\_heap\_size) | n/a | `string` | `"1"` | no |
| <a name="input_neo4j_access_external_name"></a> [neo4j\_access\_external\_name](#input\_neo4j\_access\_external\_name) | External access firewall rule name used by this Terraform deployment | `string` | `"neo4j-causal-cluster-access-external"` | no |
...
| <a name="input_neo4j_access_internal_name"></a> [neo4j\_access\_internal\_name](#input\_neo4j\_access\_internal\_name) | Internal access firewall rule name used by this Terraform deployment | `string` | `"neo4j-causal-cluster-access-internal"` | no |
| <a name="input_neo4j_access_internal_ports"></a> [neo4j\_access\_internal\_ports](#input\_neo4j\_access\_internal\_ports) | Internal access firewall rule ports used by this Terraform deployment | `list(string)` | <pre>[<br>  "5000",<br>  "6000",<br>  "7000",<br>  "7687"<br>]</pre> | no |
...
| <a name="input_neo4j_licenses"></a> [neo4j\_licenses](#input\_neo4j\_licenses) | Neo4j License key names used to upload to GCS in this Terraform deployment | `list(string)` | <pre>[<br>  "bloom.txt"<br>]</pre> | no |
| <a name="input_neo4j_plugins"></a> [neo4j\_plugins](#input\_neo4j\_plugins) | Neo4j Plugin package names used to upload to GCS in this Terraform deployment | `list(string)` | <pre>[<br>  "apoc-4.3.0.4-all.jar",<br>  "google-cloud-storage-dependencies-3.5-apoc.jar",<br>  "neo4j-bloom-1.9.0.zip"<br>]</pre> | no |
...
| <a name="input_private_ip_google_access"></a> [private\_ip\_google\_access](#input\_private\_ip\_google\_access) | Private IP Google Access of the VM being provisioned by this Terraform deployment | `string` | `"true"` | no |
| <a name="input_private_zone_description"></a> [private\_zone\_description](#input\_private\_zone\_description) | Private DNS setup description | `string` | `"Private zone DNS to support Causal Cluster address resolution"` | no |
| <a name="input_private_zone_dns"></a> [private\_zone\_dns](#input\_private\_zone\_dns) | Private DNS setup for setting up Causal Cluster resolver | `string` | `"neo4j.causal-cluster.com."` | no |
| <a name="input_project"></a> [project](#input\_project) | The project id where the VPC being provioned | `string` | n/a | yes |
| <a name="input_recordset_name"></a> [recordset\_name](#input\_recordset\_name) | Private DNS setup recordset name | `string` | `"neo4j.causal-cluster.com."` | no |
